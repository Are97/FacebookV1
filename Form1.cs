...
namespace FacebookV1
{
    public partial class Form1 : Form
    {
        ...
        {    
            string queryString =
                "select contras, idpersona, nombre, correo, apellido from persona where correo = '"+textBoxCorreoIS.Text+"'";
        ...
        private void ReadSingleRow(IDataRecord record)
        {
            ...
            {
                FormPagPrincipal fpp = new FormPagPrincipal(String.Format("{0}", record[1]), String.Format("{0}", record[2]), String.Format("{0}", record[3]), String.Format("{0}", record[0]), String.Format("{0}", record[4]));
                this.Hide();
                fpp.Show();
            }
            else {
